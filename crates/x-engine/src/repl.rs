...
use x_protocol::{crossterm::style::{Stylize, StyledContent}, ShellErr};
...
pub fn repl(input: &mut Input) -> String {
    ...
    format!("{} ", output.iter().map(|s| { s.to_string() }).collect::<Vec<_>>().join(""))
}

fn error_header(e: ShellErr, raw_input: &String, output: &mut Vec<StyledContent<String>>, parser: &mut Parser) -> Option<StyledContent<String>> {
    match e {
        x_protocol::ShellErr::Syntax(range, _) => Some(raw_input[range].to_string().red()),
        x_protocol::ShellErr::UnterminatedStr(range) => {
            Some(format!(
                "{}{}",
