...
impl<'a> Parser<'a> {
    pub fn function_syntax(&mut self) -> Result<AST> {
        let name = self.eat_token_eq_default(|token| { if let Tokens::Ident(_) = token.ty { true } else { false } }, "Missing function name")?;
    ...
    fn parameters(&mut self) -> Result<Parameters> {
        let left = self.eat_token_eq_default(|token| {
        ...
        let right = loop {
            ...
            let variable = self.eat_token_eq(|token| {
                ...
                |_| { x_protocol::ShellErr::Unterminated(left.span.clone(), "Missing right parentheses brackets.".into()) },
                Some(|s: String| { s.blue() })
            )?;
            variables.push(variable);
        };

        debug!("Parse function parameter [{}]", variables.iter().map(|v| { v.ty.to_string() }).collect::<Vec<String>>().join(", "));
